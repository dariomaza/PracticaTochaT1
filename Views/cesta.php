...
<body>
    ...
    <div class="container">
        ...
        if (isset($idCesta)) {
            ...
            if ($resultado->num_rows === 0) {
                ...
                $totalCesta = 0;
                while ($fila = $resultado->fetch_assoc()) {
                    echo '<tr><td>' ?> <img src="<?php echo $fila["imagen"] ?>" alt="" width="200px" height="200px"> 
                    <?php echo '</td>';
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["f"])) {
            ...
            $cont = 1;
            while ($fila = $resultado->fetch_assoc()) {
                $idProductoTemp = $fila["idProducto"];
                $precioUniTemp = $fila["precioProducto"];
                $cantidadTemp = $fila["cantidad"];

                $idProducto = intval($idProductoTemp);
